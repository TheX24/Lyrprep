document.addEventListener("DOMContentLoaded", async () => {
    ...
    const addLoadedSegments = () => {
        loadedSegments++;
        if (loadedSegmentsSpan) loadedSegmentsSpan.textContent = loadedSegments.toString();
    }

    if (segmentsCountSpan) segmentsCountSpan.textContent = segments.length.toString();
    ...
    // console.log(`Segment loader: Appending scripts`);
    {
        import("./app").then(() => {
            ...
            initLoaderTitle.innerHTML = `Requesting hCaptcha site key`
        })
    }
